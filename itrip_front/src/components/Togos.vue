<template>
  <div class="MyTrip">
    ...
    <div
      v-if="isLocked"
      style="position: absolute; background-color:rgb(250,250,250);right: 10px; top: 0px;"
    >
      ...
      <div class="tripName">
        <div style="width:100%;">
          ...
          <el-input
            ...
            placeholder="我的旅行"
            v-model="tripName"
            @change="$emit('changeName', tripName)"
          ></el-input>
        </div>
      </div>
      <div class="tripDate" style="display:flex;justify-content:space-between;">
        ...
        <div class="mt-2 mr-1 save-trip">
          <i
            ...
            @click="saveTripAsPdf"
            style="color:#8a8d91;font-size:25px;cursor: pointer;"
          ></i>
          <AddMemberPopover
            id="pc-addMember-popover"
            v-model="memberEmail"
        ...
        <el-dropdown ref="dropdown" placement="bottom-start" trigger="click">
          <span>
            <i
              title="功能列"
              class="pr-2 fas fa-ellipsis-h"
              style="font-size:25px;color:#8a8d91;cursor: pointer;"
            ></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item>
              <i
                ...
                class="fas fa-file-pdf"
                @click="saveTripAsPdf"
                style="color:#8a8d91;font-size:15px;cursor:pointer;"
              >匯出成PDF</i>
            </el-dropdown-item>
            <el-dropdown-item>
              <div class="pl-3 row">
                <AddMemberPopover
                  id="mobile-addMember-popover"
                  v-model="memberEmail"
                  :memberEmails="memberEmails"
                  @getCurrentMembers="getCurrentMembers"
                  @addMember="addMember"
    ...
    <div class="tab-container" style="height:75vh;">
      ...
      >
        ...
        <b-tab
          v-for="i in tabs"
          :key="'tab' + i"
          style="height: 55vh; width: 100%; overflow-Y: scroll;"
        >
          <template slot="title">
            {{ 'Day ' + (i+1) }}
          ...
          <div style="height: 65vh; width: 100%;">
            <draggable v-model="togos_prop" ghost-class="ghost" @end="onEnd">
              <transition-group type="transition" name="flip-list" :key="update">
                <div
                  class="togoContainer sortable"
                  :key="index"
                  v-for="(togo,index) in togos_prop"
                  overflow:auto
                >
...
export default {
  ...
  methods: {
    ...
    isTravelTimeShown(index) {
      if (
        this.travelInfos == undefined ||
        this.travelInfos.length == 0 ||
        index >= this.travelInfos.length
      ) {
        return false;
      }
      if (
        index < this.togos.length - 1 &&
        this.travelInfos[index] != undefined
      ) {
        return true;
      }
      return false;
    ...
    addMember: function() {
      ...
      if (!this.memberEmails.includes(this.memberEmail)) {
        ...
        apiFindMemberByMail(this.memberEmail, this.$store.state.userToken).then(
          function(res) {
            //console.log(res)
