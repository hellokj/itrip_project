    ...
    <div class="px-2 py-2 togo-item">
        <el-card :body-style="{ padding: '0px' }" shadow="hover">
            <div class="card-container">
                ...
                <div class="info-col">
                    <div class="name-container" style="text-align:left;">
                        <b>{{togo.name}}</b>
                    </div>
                    <div
                        style="text-align:left; margin-right: 5px; word-break: break-all;"
                    >{{ getAddress()}}</div>
                    <div
                        style="text-align:left; margin-right: 5px; word-break: break-all;"
                    >{{ stayTimeFormat(togo.startTime, togo.endTime)}}</div>
                </div>
            </div>
        </el-card>
    </div>
...
export default {
    ...
    methods: {
        ...
        stayTimeFormat: function(startTime, endTime) {
            return String(
                this.startTimeFormat(startTime) +
                    " ~ " +
                    this.startTimeFormat(endTime)
            );
        },
        getAddress() {
    ...
    computed: {
        srcFunc: function() {
            let src;
            if (
                !Object.keys(this.togo).includes("images") ||
                this.togo.images.length == 0
            ) {
                src = this.notFound;
            } else {
        ...
        stopTimeString: function() {
            let string = "";
            string =
                this.togo.stopTime.hrs.toString().padStart(2, "0") +
                ":" +
                this.togo.stopTime.mins.toString().padStart(2, 0);
            return string;
        },
        memo: function() {
            if (
                Object.keys(this.togo).includes("memo") &&
                this.togo.memo.length > 0
            ) {
                return this.togo.memo;
            }
            return null;
